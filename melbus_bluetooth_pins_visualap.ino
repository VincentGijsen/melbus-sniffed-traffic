...
//  {0xC0, 0x1C, 0x70, 0x02, 0x80, 0x01 },  // 5 (we respond 0x10) power off?
  ...
  {0x00, 0x1E, 0xEC }                       //12 this will also occur when attaching text, very similar! to case 0!
...
void loop() {
        ...
        if (lastByte == commands[cmd][byteCounter]) {
          ...
          if ((matching[cmd] == cmdLen[cmd]) && (byteCounter + 1 == cmdLen[cmd])) {
            ...
            switch (cmd) {
              ...
              case 12:
                 //wait for C7 and respond with same
                while (!(PIND & (1 << MELBUS_BUSY))) {
                  if (byteIsRead) {
                    byteIsRead = false;
                    //                    byteCounter++;
                    //                    lastByte = melbus_ReceivedByte;
                    //                    melbus_log[byteCounter] = lastByte;
                    if (melbus_ReceivedByte == 0xC7) {
                      byteToSend = 0xC7;
                      SendByteToMelbus();
                      if (initMasterReq){
                      SendT2();
                      initMasterReq = false;
                      Serial.print("init req master");
                      }
  ...
  if (flag) {
    ...
    if(initMasterReq){
      //we need this to do pre-startup configuration
      reqMaster(); //should trigger 00 1e ec! ->case 12 
